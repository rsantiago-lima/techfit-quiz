export async function fetchResults() {
  const results = await fetch("http://localhost:3000/results").then(
...
export async function findResult(resultId) {
  return await fetch(`http://localhost:3000/results/${resultId}`).then(
    (response) => response.json()
  );
}
...
export async function deleteResult(resultId) {
  await fetch(`http://localhost:3000/results/${resultId}`, {
    method: "DELETE",
  });
}
...
export async function fetchQuestions() {
  return await fetch("http://localhost:3000/questions").then((response) =>
    response.json()
  );
}
