...
export async function loadQuestions(quizElement) {
  ...
  questions.forEach((question, index) => {
    ...
    if (index > 0) {
      const goBackBtn = button("Voltar à pergunta anterior", {
...
async function calculateResults(questions, answers) {
  ...
  results.forEach(
    (result) => (resultsCount[result] = (resultsCount[result] || 0) + 1)
  );
...
function scrollToNextQuestion() {
  window.scrollBy({
    top: document.querySelector(".question").scrollHeight,
    behavior: "smooth",
  });
}
